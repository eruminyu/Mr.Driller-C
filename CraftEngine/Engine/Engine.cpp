...
namespace Craft
{
	...
	{
		// 실행 파일과 함께 배포된 Config 폴더에서 설정을 읽는다.
		std::filesystem::path settingPath = L"Config/Setting.txt";
		wchar_t executablePath[MAX_PATH] = {};
		const DWORD pathLength = GetModuleFileNameW(
			nullptr, executablePath, MAX_PATH);
		if (pathLength > 0 && pathLength < MAX_PATH)
		{
			settingPath = std::filesystem::path(executablePath).parent_path()
				/ L"Config" / L"Setting.txt";
		}
